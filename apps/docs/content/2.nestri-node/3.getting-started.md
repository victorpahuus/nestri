...
```bash
podman run --rm -it --shm-size=1g --device nvidia.com/gpu=0 --security-opt=label=disable -e NESTRI_ROOM=your_session_id> -e RESOLUTION=1920x1080 -e FRAMERATE=60 -e RELAY_URL='<relay_url>' -e NESTRI_PARAMS='--verbose=true --video-codec=h264 --video-bitrate=4000 --video-bitrate-max=6000 --gpu-card-path=/dev/dri/card1' --name nestri -d -v "$(pwd)":/mnt/game/ ghcr.io/datcaptainhorse/nestri-cachyos:latest-noavx2
```
